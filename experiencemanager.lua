...
function ExperienceManager:get_contract_xp_by_stars(job_id, job_stars, risk_stars, professional, job_days, extra_params)
	...
	local xp, tbl = data(self, job_id, job_stars, risk_stars, professional, job_days, extra_params)

	for id, value in pairs(tbl) do
		tbl[id] = 0
	end

	tbl[1] = offshore_rate_exp(risk_payout[1])
	tbl[2] = offshore_rate_exp(risk_payout[3])
